...
(function() {
    ...
    const CONFIG = {
        // --- General & Version ---
        SCRIPT_VERSION: '4.3',
        SCRIPT_ID_PREFIX: 'prodHelper_v4_3_', // Updated for v4.3
        ...
        STORAGE_KEY_THEME_PREFERENCE: 'themePref_global_v43', // Global, versioned
        STORAGE_KEY_STATS_CONFIG: 'statsConfig_global_v43',   // Global, versioned
        STORAGE_KEY_MINIMAL_STATS_TYPE_CONFIG: 'minimalStatsTypeConfig_global_v43', // New for v4.3, global, versioned
        ...
        updateDisplay: () => {
            ...
            if (detailedTypesConfig.length > 0) {
                ...
                detailedTypesConfig.forEach(itemConf => {
                    ...
                    if (countForType > 0) {
                        if (state.minimalStatsShowTotal) {
                             detailedLine1.push(`${itemConf.label || itemConf.type.charAt(0)}${countForType}`);
                        }
                        if (state.minimalStatsShowPerHour) {
                            const perHourForType = (hoursWorked > 0.001) ? (countForType / hoursWorked) : 0;
                            detailedLine2.push(`${itemConf.label || itemConf.type.charAt(0)}${perHourForType.toFixed(1)}${T('stats_perHour')}`);
                        }
                    }
                });
    ...
    EventHandlers = {
        ...
        handleStatsConfigChange: (event) => {
            ...
            StorageManager.saveStatsConfig(); DepartmentManager.saveDepartmentInfo(); // Save new dept status
