...
(function() {
    ...
    const CONFIG = {
        // --- General & Version ---
        SCRIPT_VERSION: '4.2',
        SCRIPT_ID_PREFIX: 'prodHelper_v4_2_',
        ...
        MINIMAL_STATS_SHOW_SECONDARY_DEFAULT: true, // New: for secondary info tabs
        ...
        TAB_IDENTIFICATION_MODES: [ // These define theme colors primarily
        ...
        // --- Department Logic (now more of a default suggestion provider) ---
        DEPARTMENTS: {
        ...
        // --- Исправлено: DEFAULT_STATS_CONFIG теперь является getter'ом ---
        ...
        STORAGE_KEY_THEME_PREFERENCE: 'themePref_global_v42', // Added for v4.1
        STORAGE_KEY_STATS_CONFIG: 'statsConfig_global_v42', // Added for v4.1
        ...
        LANG_STRINGS: {
            ...
            pl: {
                ...
                stats_format_total_items_bare: (primaryCounts, secondaryCounts, T) => { // T is a shorthand for Utils.getLangString
                    let primaryStr = "";
                    if (primaryCounts.types.length > 0 && primaryCounts.TOTAL > 0) {
                        const pParts = [];
                        if (primaryCounts.CRET > 0) pParts.push(`${primaryCounts.CRET}C`); if (primaryCounts.WHD > 0) pParts.push(`${primaryCounts.WHD}W`);
                        if (primaryCounts.REFURB > 0) pParts.push(`${primaryCounts.REFURB}R`); if (primaryCounts.UNKNOWN > 0) pParts.push(`${primaryCounts.UNKNOWN}N`); // N for Nieznany
                        primaryStr = `[${pParts.join(', ')} = ${primaryCounts.TOTAL}]`;
                    } else { primaryStr = `[0 ${T('stats_items')}]`; }
                    let secondaryStr = "";
                    if (secondaryCounts.types.length > 0 && secondaryCounts.TOTAL > 0) {
    ...
    var UIManager; // Forward declaration for JSDoc/IntelliSense in EventHandlers, etc.
    var SettingsPanel; // Forward declaration
    var EventHandlers; // Forward declaration
    ...
    const state = {
        ...
        statsConfig: JSON.parse(JSON.stringify(CONFIG.DEFAULT_STATS_CONFIG)), // Deep copy will now work with the getter
    ...
    const StorageManager = {
        ...
        loadAllState: () => {
            ...
            else state.statsConfig = JSON.parse(JSON.stringify(CONFIG.DEFAULT_STATS_CONFIG)); // Will use getter now
    ...
    const MinimalStatsDisplay = {
        ...
        updateDisplay: () => {
            ...
            if (state.minimalStatsShowSecondary && aggregatedCounts.secondary.TOTAL > 0 && !state.minimalStatsShowTotal) { let secondaryOnlyDisplay = { CRET:0,WHD:0,REFURB:0,UNKNOWN:0,TOTAL:aggregatedCounts.secondary.TOTAL, types:aggregatedCounts.secondary.types}; if (aggregatedCounts.secondary.CRET > 0) secondaryOnlyDisplay.CRET = aggregatedCounts.secondary.CRET; if (aggregatedCounts.secondary.WHD > 0) secondaryOnlyDisplay.WHD = aggregatedCounts.secondary.WHD; if (aggregatedCounts.secondary.REFURB > 0) secondaryOnlyDisplay.REFURB = aggregatedCounts.secondary.REFURB; if (aggregatedCounts.secondary.UNKNOWN > 0) secondaryOnlyDisplay.UNKNOWN = aggregatedCounts.secondary.UNKNOWN; displayTextParts.push( T('stats_format_total_items_bare', { CRET:0,WHD:0,REFURB:0,UNKNOWN:0,TOTAL:0, types:[] }, secondaryOnlyDisplay, T).replace(/^\[0\s\w+\]/, '') );}
    ...
    UIManager = {
        ...
        init: () => { UIManager.buildInitialUI(); UIManager.setInitialUIValues(); ResizableItemsManager.initPanelResizing(state.uiContainer); UIManager._isInitialized = true; UIManager.applyThemeStyles(); }, // Apply theme after building
        ...
        buildMainPanel: () => {
            ...
            state.dom.rightPane = Utils.createDOMElement('div', { className: 'ph-right-pane ph-interactive-parent', style: { display: 'flex', flexDirection: 'column', flexGrow: 1, overflowY: 'auto', paddingLeft: `${CONFIG.DIVIDER_WIDTH_PX / 2}px`, minWidth: `${CONFIG.RIGHT_PANE_MIN_WIDTH_PERCENT}%` /* fontSize set by theme */ }});
            ...
            state.dom.lastActionTimerDisplay = Utils.createDOMElement('div', { id: 'lastActionTimer', style: { opacity: '0.7' /* fontSize set by theme */ }});
            state.dom.realTimeClock = Utils.createDOMElement('div', { id: 'realTimeClock', style: { opacity: '0.5' /* fontSize set by theme */ }});
        ...
        applyThemeStyles: () => {
            ...
            injectGlobalStyles(); // Re-inject theme-dependent styles like scrollbars
    ...
    EventHandlers = {
        ...
        handleStatsConfigChange: (event) => {
            ...
            if (SettingsPanel.isInitialized()) SettingsPanel.populateDepartmentInfo();
        },
    ...
    function actualInit() {
        ...
        UIManager.init(); MinimalStatsDisplay.init(); UIManager.setInitialUIValues(); // This will call applyThemeStyles()
