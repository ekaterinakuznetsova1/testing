...
(function() {
    ...
    const CONFIG = {
        // --- General & Version ---
        SCRIPT_VERSION: '4.6', // Updated for v4.6
        SCRIPT_ID_PREFIX: 'prodHelper_v4_6_', // Updated for v4.6
        ...
        THEMES: {
            light: {
                ...
                MINIMAL_STATS_TEXT_COLOR: 'rgba(70, 70, 70, 0.9)',
            ...
            dark: {
                ...
                MINIMAL_STATS_TEXT_COLOR: 'rgba(190, 190, 190, 0.85)',
        ...
        MINIMAL_STATS_FONT_SIZE_EM: 1.1,
        MINIMAL_STATS_LINE_HEIGHT_EM: 1.3, // Adjusted for potentially three lines
        ...
        // --- Department Logic ---
        DEPARTMENTS: { // Values are now the direct string identifiers
        ...
        STORAGE_EVENT_DEBOUNCE_MS: 100, // New for v4.6
        ...
        // --- Storage Keys (Updated for v4.6) ---
        STORAGE_KEY_MAIN_SETTINGS: 'mainCfg', // Remains per-tab ID if session, or global if not
        STORAGE_KEY_CUSTOM_THEMES: 'customThemes_global',
        STORAGE_KEY_UI_SIZE_POS: 'uiSizePos', // Global for UI size
        STORAGE_KEY_MINIMAL_STATS_POS: 'minimalStatsPos', // Global for minimal stats position
        STORAGE_KEY_DEPARTMENT_INFO: 'departmentInfo', // Global department info
        STORAGE_KEY_LANGUAGE: 'languagePref_global',
        STORAGE_KEY_THEME_PREFERENCE: 'themePref_global_v46',       // Versioned for v4.6
        STORAGE_KEY_STATS_CONFIG: 'statsConfig_global_v46',         // Versioned for v4.6
        STORAGE_KEY_MINIMAL_STATS_TYPE_CONFIG: 'minimalStatsTypeConfig_global_v46', // Versioned for v4.6
        USE_SESSION_STORAGE_FOR_UI_STATE: true, // For mainSettings (per tab)
        ...
        LANG_STRINGS: {
            en: {
                ...
                "statsConfig_tabType_C-RET": "C-RET Tab",
                statsConfig_tabType_WHD: "WHD Tab",
                statsConfig_tabType_REFURB: "REFURB Tab",
                statsConfig_tabType_General: "Other Tabs", // Corrected key for "Other Tabs"
                ...
                depSummary_UG_logic: "C+W+R", depSummary_BK_logic: "C+W (R info)", depSummary_RB_logic: "REFURB (WHD info)", depSummary_CR_logic: "C-RET only", depSummary_WD_logic: "WHD only", depSummary_RB_ONLY_logic: "REFURB only", depSummary_CUSTOM_logic: "Custom Rules", depSummary_UNKNOWN_logic: "", depSummary_DETERMINING_logic: "",
                ...
                stats_format_total_items_bare: (primaryCounts, secondaryCounts, T) => {
                    let primaryStr = "";
            ...
            pl: {
                ...
                "statsConfig_tabType_C-RET": "Karta C-RET",
                statsConfig_tabType_WHD: "Karta WHD",
                statsConfig_tabType_REFURB: "Karta REFURB",
                statsConfig_tabType_General: "Inne Karty", // Corrected key for "Inne Karty"
                ...
                depSummary_UG_logic: "C+W+R", depSummary_BK_logic: "C+W (R info)", depSummary_RB_logic: "REFURB (WHD info)", depSummary_CR_logic: "Tylko C-RET", depSummary_WD_logic: "Tylko WHD", depSummary_RB_ONLY_logic: "Tylko REFURB", depSummary_CUSTOM_logic: "Reguły własne", depSummary_UNKNOWN_logic: "", depSummary_DETERMINING_logic: "",
                ...
                stats_format_total_items_bare: (primaryCounts, secondaryCounts, T) => {
                    ...
                    if (primaryCounts.types.length > 0 && primaryCounts.TOTAL > 0) {
                        const pParts = [];
                        if (primaryCounts.CRET > 0) pParts.push(`${primaryCounts.CRET}C`); if (primaryCounts.WHD > 0) pParts.push(`${primaryCounts.WHD}W`);
                        if (primaryCounts.REFURB > 0) pParts.push(`${primaryCounts.REFURB}R`); if (primaryCounts.UNKNOWN > 0) pParts.push(`${primaryCounts.UNKNOWN}N`); // N for Nieznane
                        primaryStr = `[${pParts.join(', ')} = ${primaryCounts.TOTAL}]`;
                    } else { primaryStr = `[0 ${T('stats_items')}]`; }
                    let secondaryStr = "";
                    if (secondaryCounts.types.length > 0 && secondaryCounts.TOTAL > 0) {
                        const sParts = [];
                        if (secondaryCounts.CRET > 0) sParts.push(`${secondaryCounts.CRET}C`); if (secondaryCounts.WHD > 0) sParts.push(`${secondaryCounts.WHD}W`);
                        if (secondaryCounts.REFURB > 0) sParts.push(`${secondaryCounts.REFURB}R`); if (secondaryCounts.UNKNOWN > 0) sParts.push(`${secondaryCounts.UNKNOWN}N`); // N for Nieznane
                        secondaryStr = ` (+${sParts.join(', ')})`;
                    } return `${primaryStr}${secondaryStr}`;
                },
            }
    ...
    const Utils = {
        ...
        getLangString: (key, ...args) => {
            ...
            if (typeof str === 'undefined') {
                ...
                if (typeof str === 'undefined') {
                    if (key && (key.includes("depSummary_") && key.endsWith("_logic")) ) return "";
            ...
            if (typeof str === 'function') {
                try { return str(...args); } catch (e) { Utils.error(`Error executing lang string function for key "${key}":`, e); return `[Error in lang fn: ${key}]`; }
            }
            if (args.length > 0 && typeof str === 'string' && args[0] !== null) {
    ...
    const StorageManager = {
        ...
        writeCurrentTabDataToLocalStorage: () => {
            ...
            try {
                const tabData = {
                    ...
                    contributesToAggregation: state.currentTabContributesToAggregation,
                    department: state.currentDepartment,
                    timestamp: Date.now()
                };
                if (CONFIG.DEBUG_MODE) {
                    Utils.debug(`writeCurrentTabDataToLocalStorage: Writing data for tab ${state.currentTabId}:`, JSON.parse(JSON.stringify(tabData)));
                }
                localStorage.setItem(CONFIG.SCRIPT_ID_PREFIX + CONFIG.MULTI_TAB_STORAGE_PREFIX + state.currentTabId, JSON.stringify(tabData));
            } catch (e) {
                Utils.error('Error writing tab data to localStorage:', e);
            }
        },
        readAllTabsDataFromLocalStorage: (isInitialLoad = false) => {
            ...
            if (CONFIG.DEBUG_MODE) {
                Utils.debug("readAllTabsDataFromLocalStorage: --- START DEBUG ---");
                Utils.debug("readAllTabsDataFromLocalStorage: Current state.otherTabsData (before read):", JSON.parse(JSON.stringify(state.otherTabsData)));
                let allMultiTabKeys = {};
                try {
                    for (let i = 0; i < localStorage.length; i++) {
                        const key = localStorage.key(i);
                        if (key && key.startsWith(CONFIG.SCRIPT_ID_PREFIX + CONFIG.MULTI_TAB_STORAGE_PREFIX)) {
                            allMultiTabKeys[key] = localStorage.getItem(key);
                        }
                    }
                } catch (e) { Utils.error("readAllTabsDataFromLocalStorage: Error reading localStorage keys for debug:", e); }
                Utils.debug("readAllTabsDataFromLocalStorage: Raw localStorage content for MULTI_TAB_STORAGE_PREFIX:", allMultiTabKeys);
                Utils.debug(`readAllTabsDataFromLocalStorage: Reading all tabs data. Current tab ID: ${state.currentTabId}. Initial load: ${isInitialLoad}`);
            }
            ...
            try {
                    ...
                    if (key && key.startsWith(CONFIG.SCRIPT_ID_PREFIX + CONFIG.MULTI_TAB_STORAGE_PREFIX)) {
                        ...
                        if (itemJson) {
                            try {
                                ...
                                if (CONFIG.DEBUG_MODE) Utils.debug(`readAllTabsDataFromLocalStorage: Parsed itemData for key ${key}:`, JSON.parse(JSON.stringify(itemData)));

                                if (now - (itemData.timestamp || 0) > CONFIG.MULTI_TAB_DATA_TTL_MS) {
                                    if (CONFIG.DEBUG_MODE) Utils.debug(`readAllTabsDataFromLocalStorage: Tab data for ${itemData.tabId} (key: ${key}) expired. Removing.`);
                                ...
                                if (itemData.tabId === state.currentTabId) {
                                    if (isInitialLoad) {
                                        ...
                                        if (CONFIG.DEBUG_MODE) Utils.debug(`readAllTabsDataFromLocalStorage: Initial load for current tab ${state.currentTabId}: Clicks=${state.clicksForThisTab}`);
                                    }
                                    newOtherTabsData[itemData.tabId] = {
                                        ...itemData,
                                        clicks: state.clicksForThisTab,
                                        lastActionTimestamp: state.lastActionTimestampForThisTab,
                                        contributesToAggregation: state.currentTabContributesToAggregation,
                                    ...
                                    if (CONFIG.DEBUG_MODE) Utils.debug(`readAllTabsDataFromLocalStorage: Data for CURRENT tab ${itemData.tabId} being merged into newOtherTabsData:`, JSON.parse(JSON.stringify(newOtherTabsData[itemData.tabId])));
                                } else {
                                    newOtherTabsData[itemData.tabId] = itemData;
                                    if (CONFIG.DEBUG_MODE) Utils.debug(`readAllTabsDataFromLocalStorage: Data for OTHER tab ${itemData.tabId} added to newOtherTabsData:`, JSON.parse(JSON.stringify(itemData)));
                                }
                            } catch (parseError) {
                                Utils.error(`Error parsing localStorage key ${key}:`, parseError);
                                localStorage.removeItem(key);
                            }
                        }
                    }
                ...
                Utils.error('Error reading from localStorage multi-tab sync:', e);
            }

            if (CONFIG.DEBUG_MODE) {
                Utils.debug("readAllTabsDataFromLocalStorage: Constructed newOtherTabsData (before assignment):", JSON.parse(JSON.stringify(newOtherTabsData)));
                Utils.debug("readAllTabsDataFromLocalStorage: --- END DEBUG ---");
            }

            state.otherTabsData = newOtherTabsData;
            // Utils.debug("Updated state.otherTabsData:", JSON.parse(JSON.stringify(state.otherTabsData))); // Already logged more comprehensively above
            ...
            if (!isInitialLoad && typeof UIManager !== 'undefined' && UIManager.isInitialized()) {
                ...
                if (state.settingsPanelVisible && SettingsPanel.isInitialized()) SettingsPanel.updateOtherTabsSettingsDisplay();
            }
        }
    };
    ...
    const DepartmentManager = {
        ...
        _setDepartmentProfile: (dept, applyDefaultStats = true) => {
            ...
            if (state.initialized) { UIManager.updateStatisticsDisplay(); MinimalStatsDisplay.updateDisplay(); if (state.settingsPanelVisible && SettingsPanel.isInitialized()) { SettingsPanel.populateDepartmentInfo(); SettingsPanel.populateStatsConfigFields(); SettingsPanel.populateMinimalStatsTypeDisplayConfigFields(); }}
        },
    ...
    MinimalStatsDisplay = {
        ...
        init: () => {
            ...
            state.dom.minimalStatsContent = Utils.createDOMElement('span');
        ...
        getDefaultTypeConfig: () => {
            return [
                ...
                { type: CONFIG.TAB_TYPES.UNKNOWN,visible: false, order: 3, label: 'U' } // "General" maps to "UNKNOWN"
            ];
        },
        updateTheme: () => {
        ...
        updateDisplay: () => {
            ...
            if (!state.shiftStartTime) {
                state.dom.minimalStatsContent.textContent = `${Utils.getLangString('error')}: Shift not started`;
                return;
            }
            ...
            const hoursWorked = effectiveWorkMs > 0 ? effectiveWorkMs / (1000 * 60 * 60) : 0;

            let line1_overall = "";
            let line1_details_parts = [];
            let line2_overall = "";
            let line2_details_parts = [];
            let line3_department = "";

            if (CONFIG.DEBUG_MODE) {
                Utils.debug("MinimalStatsDisplay.updateDisplay: Aggregated counts used:", JSON.parse(JSON.stringify(aggregatedCounts)));
                Utils.debug("MinimalStatsDisplay.updateDisplay: Effective Work MS:", effectiveWorkMs, "Hours Worked:", hoursWorked);
            }

            if (state.minimalStatsShowTotal) {
                const primaryTotalItems = aggregatedCounts.primary.TOTAL;
                line1_overall = (primaryTotalItems > 0) ? String(primaryTotalItems) : `0 ${T('stats_items')}`;
            }
            ...
            if (state.minimalStatsShowPerHour) {
                ...
                line2_overall = (primaryTotalPerHour > 0) ? `${primaryTotalPerHour.toFixed(1)}${T('stats_perHour')}` : `0.0${T('stats_perHour')}`;
            }

            const visibleTypesConfig = state.minimalStatsTypeDisplayConfig
                .filter(item => item.visible)
                .sort((a, b) => a.order - b.order);

            if (CONFIG.DEBUG_MODE) {
                Utils.debug("MinimalStatsDisplay.updateDisplay: Visible Types Config for details:", JSON.parse(JSON.stringify(visibleTypesConfig)));
            }

            if (visibleTypesConfig.length > 0) {
                visibleTypesConfig.forEach(itemConf => {
                    const countForType = aggregatedCounts.allTabs[itemConf.type] || 0;
                    const staticLabel = itemConf.label;

                    if (state.minimalStatsShowTotal) {
                        line1_details_parts.push(`${staticLabel}${countForType}`);
                    }
                    if (state.minimalStatsShowPerHour) {
                        const perHourForType = (hoursWorked > 0.001 && countForType > 0) ? (countForType / hoursWorked) : 0;
                        line2_details_parts.push(`${staticLabel}${perHourForType.toFixed(1)}`);
                    }
                });
            }
            ...
            if (state.currentDepartment &&
                ...
                state.currentDepartment !== CONFIG.DEPARTMENTS.UNKNOWN) {
                const deptProfileName = DepartmentManager.getDepartmentDisplayName(state.currentDepartment);
                const deptLogicKey = `depSummary_${state.currentDepartment}_logic`;
                const deptLogicSummary = T(deptLogicKey, null);
                if (deptLogicSummary && !deptLogicSummary.startsWith("[Missing") && deptLogicSummary.trim() !== "") {
                    line3_department = `${deptProfileName}: ${deptLogicSummary}`;
                } else {
                    line3_department = deptProfileName;
                }
            } else if (state.currentDepartment === CONFIG.DEPARTMENTS.DETERMINING) {
                 line3_department = T('depName_DETERMINING');
            }

            let finalDisplayParts = [];
            let assembledLine1 = "";
            if (line1_overall) assembledLine1 += line1_overall;
            if (line1_details_parts.length > 0) {
                assembledLine1 += (assembledLine1 ? " " : "") + `[${line1_details_parts.join(', ')}]`;
            }
            if (assembledLine1) finalDisplayParts.push(assembledLine1);

            let assembledLine2 = "";
            if (line2_overall) assembledLine2 += line2_overall;
            if (line2_details_parts.length > 0) {
                assembledLine2 += (assembledLine2 ? " " : "") + `[${line2_details_parts.join(', ')}]${T('stats_perHour')}`;
            }
            if (assembledLine2) finalDisplayParts.push(assembledLine2);

            if (line3_department) {
                finalDisplayParts.push(line3_department);
            }

            const finalHtml = finalDisplayParts.join('<br>');

            if (finalDisplayParts.join('').trim() === "") {
                if (state.currentDepartment === CONFIG.DEPARTMENTS.DETERMINING) {
                    state.dom.minimalStatsContent.innerHTML = T('depName_DETERMINING');
                } else {
                    state.dom.minimalStatsContent.innerHTML = DepartmentManager.getDepartmentDisplayName(state.currentDepartment);
                }
                 if (CONFIG.DEBUG_MODE) Utils.debug("MinimalStatsDisplay.updateDisplay: Final HTML (fallback):", state.dom.minimalStatsContent.innerHTML.replace(/<br>/g, ' | '));
            } else {
                state.dom.minimalStatsContent.innerHTML = finalHtml;
                if (CONFIG.DEBUG_MODE) Utils.debug("MinimalStatsDisplay.updateDisplay: Final HTML:", finalHtml.replace(/<br>/g, ' | '));
            }
        },
        setVisibility: (visible) => {
            ...
            if(MinimalStatsDisplay.dragHandle) MinimalStatsDisplay.dragHandle.style.display = 'none';
        },
    ...
    const StatsCalculator = {
        getAggregatedCounts: () => {
            ...
            const allTabsRawCounts = {};
            Object.values(CONFIG.TAB_TYPES).forEach(tabTypeConst => {
                allTabsRawCounts[tabTypeConst] = 0;
            });

            if (CONFIG.DEBUG_MODE) {
                Utils.debug("StatsCalculator: Calculating aggregated counts. Incoming state.otherTabsData:", JSON.parse(JSON.stringify(state.otherTabsData)));
            }

            for (const tabId in state.otherTabsData) {
                const tab = state.otherTabsData[tabId];
                if (!tab.contributesToAggregation && tab.tabId !== state.currentTabId) { // Current tab's own clicks always contribute to its 'allTabsRawCounts'
                    if (CONFIG.DEBUG_MODE) Utils.debug(`StatsCalculator: Tab ${tabId} does not contribute to GLOBAL aggregation and is not current tab. Skipping for primary/secondary counts.`);
                }

                const clicks = parseInt(tab.clicks, 10) || 0;
                const tabType = Utils.normalizeTabType(tab.type); 

                if (CONFIG.DEBUG_MODE) Utils.debug(`StatsCalculator: Processing tab ${tabId} - Type: ${tab.type} (Normalized: ${tabType}), Clicks: ${clicks}, Contributes Globally: ${tab.contributesToAggregation}`);
                ...
                if(allTabsRawCounts.hasOwnProperty(tabType)) {
                    ...
                    Utils.warn(`StatsCalculator: tabType '${tabType}' from normalizeTabType not directly in allTabsRawCounts keys. Original tab.type: '${tab.type}'`);
                     allTabsRawCounts[CONFIG.TAB_TYPES.UNKNOWN] = (allTabsRawCounts[CONFIG.TAB_TYPES.UNKNOWN] || 0) + clicks; // Fallback
                }

                // Only consider for primary/secondary if it contributes globally OR it's the current tab (whose contribution setting is for global, not for its own type)
                if (tab.contributesToAggregation || tab.tabId === state.currentTabId) {
                    if (state.statsConfig.primaryContributingTabs.includes(tabType)) {
                        ...
                        else primaryCounts[CONFIG.TAB_TYPES.UNKNOWN] += clicks; // Should not happen if types are consistent
                    ...
                    if (state.statsConfig.secondaryInfoTabs.includes(tabType)) {
                        ...
                        else secondaryCounts[CONFIG.TAB_TYPES.UNKNOWN] += clicks; // Should not happen
                        secondaryCounts.TOTAL += clicks;
                    }
                }
            }
            // Ensure all defined types exist in counts, even if 0
            ...
            Object.values(CONFIG.TAB_TYPES).forEach(type => { if(!allTabsRawCounts.hasOwnProperty(type)) allTabsRawCounts[type] = 0; });
            
            if (CONFIG.DEBUG_MODE) {
                 Utils.debug("StatsCalculator: Aggregated Results - Primary:", JSON.parse(JSON.stringify(primaryCounts)), "Secondary:", JSON.parse(JSON.stringify(secondaryCounts)), "AllTabsRaw:", JSON.parse(JSON.stringify(allTabsRawCounts)));
            }
    ...
    UIManager = {
        ...
        updateStatisticsDisplay: () => {
            ...
            let statsHTML = `<p style="margin-bottom: 2px;">${T('stats_shift')}: <strong>${Utils.formatDateToHHMM(state.shiftStartTime)}</strong> (${T('shiftType_' + state.shiftType) || state.shiftType})</p>
                             ...
                             <div style="font-size:0.9em; margin-bottom: 3px;">${T('department')}: <strong>${DepartmentManager.getDepartmentDisplayName(state.currentDepartment)}</strong></div>
    ...
    SettingsPanel = {
        ...
        rebuildOrRelocalize: () => {
            ...
            state.dom.settingsThemeSelect = Utils.createDOMElement('select', {id: 'themeSelect', style: commonSelectStyle, className:'ph-interactive', onChange: EventHandlers.handleThemeChange }); Object.keys(CONFIG.THEMES).forEach(themeKey => state.dom.settingsThemeSelect.add(new Option(T(`theme_${themeKey}`), themeKey))); state.dom.settingsPanel.appendChild(createSection(null, [Utils.createDOMElement('label', {htmlFor: CONFIG.SCRIPT_ID_PREFIX + 'themeSelect', textContent: T('theme'), style: commonLabelStyle}), state.dom.settingsThemeSelect ]));
            state.dom.languageSelect = Utils.createDOMElement('select', { id: 'languageSelect', style: commonSelectStyle, className:'ph-interactive', onChange: EventHandlers.handleLanguageChange }); CONFIG.AVAILABLE_LANGUAGES.forEach(lang => state.dom.languageSelect.add(new Option(lang.name, lang.code))); state.dom.settingsPanel.appendChild(createSection(null, [Utils.createDOMElement('label', { htmlFor: CONFIG.SCRIPT_ID_PREFIX + 'languageSelect', textContent: T('language_select'), style: commonLabelStyle }), state.dom.languageSelect ]));
            state.dom.pointerEventsModeSelect = Utils.createDOMElement('select', { id: 'pointerEventsModeSelect', style: commonSelectStyle, className:'ph-interactive', onChange: EventHandlers.handlePointerModeChange }); CONFIG.POINTER_EVENTS_MODES.forEach(opt => state.dom.pointerEventsModeSelect.add(new Option(T(opt.text_key), opt.value))); state.dom.settingsPanel.appendChild(createSection('peMode_title', [ Utils.createDOMElement('label', { htmlFor: CONFIG.SCRIPT_ID_PREFIX + 'pointerEventsModeSelect', textContent: T('peMode_label'), style: commonLabelStyle }), state.dom.pointerEventsModeSelect ]));
            state.dom.settingsShiftTypeSelect = Utils.createDOMElement('select', { id: 'shiftTypeSelect', style: commonSelectStyle, className:'ph-interactive', onChange: EventHandlers.handleShiftSettingsChange }); state.dom.settingsShiftStartTimeInput = Utils.createDOMElement('input', { type: 'time', id: 'shiftStartTimeInput', style: commonInputStyle, className:'ph-interactive', onChange: EventHandlers.handleShiftSettingsChange }); state.dom.settingsLunchSelect = Utils.createDOMElement('select', { id: 'lunchSelect', style: commonSelectStyle, className:'ph-interactive', onChange: EventHandlers.handleLunchSettingChange }); state.dom.settingsPanel.appendChild(createSection('shiftLunch_title', [ Utils.createDOMElement('label', { htmlFor: CONFIG.SCRIPT_ID_PREFIX + 'shiftTypeSelect', textContent: T('shiftType'), style: commonLabelStyle }), state.dom.settingsShiftTypeSelect, Utils.createDOMElement('label', { htmlFor: CONFIG.SCRIPT_ID_PREFIX + 'shiftStartTimeInput', textContent: T('shiftStartManual'), style: {...commonLabelStyle, marginTop:'6px'} }), state.dom.settingsShiftStartTimeInput, Utils.createDOMElement('label', { htmlFor: CONFIG.SCRIPT_ID_PREFIX + 'lunchSelect', textContent: T('lunchBreak'), style: {...commonLabelStyle, marginTop:'6px'} }), state.dom.settingsLunchSelect, ]));
            ...
            const statsConfigElements = []; const buildStatsCheckbox = (typeKey, listKey) => { const rawCbId = `statsCfg_${listKey}_${typeKey.replace('-','')}`; state.dom[rawCbId] = Utils.createDOMElement('input', {type:'checkbox', id: rawCbId, style: checkboxStyle, className:'ph-interactive ph-stats-config-cb', dataset: { tabtype: typeKey, listkey: listKey }, onChange: EventHandlers.handleStatsConfigChange }); return Utils.createDOMElement('label', {htmlFor: CONFIG.SCRIPT_ID_PREFIX + rawCbId, textContent: T(`statsConfig_tabType_${typeKey}`), style:checkboxRowStyle, className:'ph-interactive'}, [state.dom[rawCbId]]); }; statsConfigElements.push(Utils.createDOMElement('p', {textContent: T('statsConfig_primaryLabel'), style: commonLabelStyle})); [CONFIG.TAB_TYPES.CRET, CONFIG.TAB_TYPES.WHD, CONFIG.TAB_TYPES.REFURB, CONFIG.TAB_TYPES.UNKNOWN].forEach(type => statsConfigElements.push(buildStatsCheckbox(type, 'primaryContributingTabs'))); statsConfigElements.push(Utils.createDOMElement('p', {textContent: T('statsConfig_secondaryLabel'), style: {...commonLabelStyle, marginTop:'8px'}})); [CONFIG.TAB_TYPES.CRET, CONFIG.TAB_TYPES.WHD, CONFIG.TAB_TYPES.REFURB, CONFIG.TAB_TYPES.UNKNOWN].forEach(type => statsConfigElements.push(buildStatsCheckbox(type, 'secondaryInfoTabs'))); state.dom.settingsPanel.appendChild(createSection('statsConfig_title', statsConfigElements));
            ...
            SettingsPanel.buildMinimalStatsTypeDisplayConfigUI(); // This will be called again but it's fine, or call populate here.
            state.dom.autoClickEnabledCheckbox = Utils.createDOMElement('input', { type: 'checkbox', id: 'autoClickEnableCb', style: checkboxStyle, className:'ph-interactive', onChange: EventHandlers.handleAutoClickSettingChange }); const autoClickLabel = Utils.createDOMElement('label', {htmlFor: CONFIG.SCRIPT_ID_PREFIX + 'autoClickEnableCb', textContent: T('automation_autoIncrement'), style:checkboxRowStyle, className:'ph-interactive'}, [state.dom.autoClickEnabledCheckbox]);
            state.dom.currentTabContributesCheckbox = Utils.createDOMElement('input', {type: 'checkbox', id: 'currentTabContributesCb', style: checkboxStyle, className:'ph-interactive', onChange: EventHandlers.handleCurrentTabAggregationChange }); const currentTabContributeLabel = Utils.createDOMElement('label', {htmlFor: CONFIG.SCRIPT_ID_PREFIX + 'currentTabContributesCb', textContent: T('automation_tabContributesToAggregation'), style: checkboxRowStyle, className:'ph-interactive' }, [state.dom.currentTabContributesCheckbox]);
            ...
            const minimalStatsElements = []; state.dom.minimalStatsEnabledCheckbox = Utils.createDOMElement('input', {type:'checkbox', id:'minimalStatsEnableCb', style: checkboxStyle, className:'ph-interactive', onChange: EventHandlers.handleMinimalStatsEnableChange}); minimalStatsElements.push(Utils.createDOMElement('label', {htmlFor: CONFIG.SCRIPT_ID_PREFIX + 'minimalStatsEnableCb', textContent: T('minimalStats_enable'), style:checkboxRowStyle, className:'ph-interactive'}, [state.dom.minimalStatsEnabledCheckbox]));
            state.dom.minimalStatsShowTotalCheckbox = Utils.createDOMElement('input', {type:'checkbox', id:'minimalStatsShowTotalCb', style: checkboxStyle, className:'ph-interactive', onChange: EventHandlers.handleMinimalStatsDetailChange}); minimalStatsElements.push(Utils.createDOMElement('label', {htmlFor: CONFIG.SCRIPT_ID_PREFIX + 'minimalStatsShowTotalCb', textContent: T('minimalStats_showTotal'), style:checkboxRowStyle, className:'ph-interactive'}, [state.dom.minimalStatsShowTotalCheckbox]));
            state.dom.minimalStatsShowPerHourCheckbox = Utils.createDOMElement('input', {type:'checkbox', id:'minimalStatsShowPerHourCb', style: checkboxStyle, className:'ph-interactive', onChange: EventHandlers.handleMinimalStatsDetailChange}); minimalStatsElements.push(Utils.createDOMElement('label', {htmlFor: CONFIG.SCRIPT_ID_PREFIX + 'minimalStatsShowPerHourCb', textContent: T('minimalStats_showPerHour'), style:checkboxRowStyle, className:'ph-interactive'}, [state.dom.minimalStatsShowPerHourCheckbox]));
        ...
        buildMinimalStatsTypeDisplayConfigUI: () => {
            ...
            state.dom.minimalStatsTypeDisplayConfigContainer.innerHTML = '';
            const T = Utils.getLangString; const TC = Utils.getThemeColor;
            ...
            state.minimalStatsTypeDisplayConfig.sort((a,b) => a.order - b.order).forEach((item, index) => {
                ...
                const typeLabelText = T(`statsConfig_tabType_${item.type}`); // e.g. "C-RET Tab"
                ...
                const rawVisibleCheckboxId = `minStatsDetailVis_${item.type.replace(/[^a-zA-Z0-9]/g, '')}`; // Sanitize ID
                const visibleCheckbox = Utils.createDOMElement('input', {
                    type: 'checkbox', id: rawVisibleCheckboxId, checked: item.visible, className: 'ph-interactive',
                    style: { marginRight: '8px', accentColor: TC('MAIN_ACCENT_COLOR'), cursor: 'pointer' },
                    dataset: { type: item.type }, onChange: EventHandlers.handleMinimalStatsTypeDetailChange
                });
                const visibleLabel = Utils.createDOMElement('label', {
                    htmlFor: CONFIG.SCRIPT_ID_PREFIX + rawVisibleCheckboxId, textContent: T('minimalStats_typeDisplay_showInDetails'),
                    style: { color: TC('SETTINGS_PANEL_TEXT_COLOR'), fontSize: '0.85em', marginRight: '10px', cursor:'pointer' }
                });

                const rawShortLabelInputId = `minStatsDetailLabel_${item.type.replace(/[^a-zA-Z0-9]/g, '')}`; // Sanitize ID
                const shortLabelInput = Utils.createDOMElement('input', {
                    type: 'text', id: rawShortLabelInputId, value: item.label, maxLength: 2, style: commonInputStyle,
                    className: 'ph-interactive', dataset: { type: item.type }, onInput: EventHandlers.handleMinimalStatsTypeDetailChange
                });
                const shortLabelLabel = Utils.createDOMElement('label', {
                    htmlFor: CONFIG.SCRIPT_ID_PREFIX + rawShortLabelInputId, textContent: T('minimalStats_typeDisplay_label'),
                    style: { color: TC('SETTINGS_PANEL_TEXT_COLOR'), fontSize: '0.85em', marginRight: '5px'}
                });
        ...
        populateDepartmentInfo: () => { if (!SettingsPanel.isInitialized() || !state.dom.departmentInfoDisplay) return; state.dom.departmentInfoDisplay.textContent = `${Utils.getLangString('department')}: ${DepartmentManager.getDepartmentDisplayName(state.currentDepartment)}`; if (state.currentDepartment === CONFIG.DEPARTMENTS.DETERMINING && state.departmentInfo.sequence.length > 0) state.dom.departmentInfoDisplay.textContent += ` (Sequence: ${state.departmentInfo.sequence.map(s=>s.type).join(' -> ')})`; },
        populateStatsConfigFields: () => {
            ...
            [CONFIG.TAB_TYPES.CRET, CONFIG.TAB_TYPES.WHD, CONFIG.TAB_TYPES.REFURB, CONFIG.TAB_TYPES.UNKNOWN].forEach(type => {
                const rawPrimaryCbId = `statsCfg_primaryContributingTabs_${type.replace('-','')}`;
                const primaryCb = state.dom[rawPrimaryCbId]; if (primaryCb) primaryCb.checked = state.statsConfig.primaryContributingTabs.includes(type);
                const rawSecondaryCbId = `statsCfg_secondaryInfoTabs_${type.replace('-','')}`;
                const secondaryCb = state.dom[rawSecondaryCbId]; if (secondaryCb) secondaryCb.checked = state.statsConfig.secondaryInfoTabs.includes(type);
            });
        },
        populateMinimalStatsTypeDisplayConfigFields: () => {
            ...
            SettingsPanel.buildMinimalStatsTypeDisplayConfigUI(); // Rebuilds the list with correct IDs and values
        },
        updateOtherTabsSettingsDisplay: () => {
            ...
            otherTabs.forEach(tabData => { const rawCheckboxId = `contribToggle_${tabData.tabId.replace(/[^a-zA-Z0-9]/g, '_')}`; const checkbox = Utils.createDOMElement('input', { type: 'checkbox', id: rawCheckboxId, checked: tabData.contributesToAggregation || false, style: checkboxStyle, dataset: { targetTabId: tabData.tabId }, className:'ph-interactive', onChange: EventHandlers.handleOtherTabAggregationChange }); const label = Utils.createDOMElement('label', { htmlFor: CONFIG.SCRIPT_ID_PREFIX + rawCheckboxId, style: checkboxRowStyle, className:'ph-interactive' }, [ checkbox, `${tabData.modeName||tabData.tabId.substring(0,12)+'...'} (${tabData.clicks || 0} ${T('stats_items')})` ]); state.dom.otherTabsSettingsContainer.appendChild(label); });
        }
    };

    EventHandlers = {
        _updateFromStorageDebounced: Utils.debounce(() => {
            if (CONFIG.DEBUG_MODE) Utils.debug("EventHandlers: Debounced storage event triggered. Reading all tabs data.");
            StorageManager.readAllTabsDataFromLocalStorage(false);
        }, CONFIG.STORAGE_EVENT_DEBOUNCE_MS),
        ...
        handleMinimalStatsDetailChange: (event) => { const checkbox = event.target; if (checkbox.id === (CONFIG.SCRIPT_ID_PREFIX + 'minimalStatsShowTotalCb')) state.minimalStatsShowTotal = checkbox.checked; else if (checkbox.id === (CONFIG.SCRIPT_ID_PREFIX + 'minimalStatsShowPerHourCb')) state.minimalStatsShowPerHour = checkbox.checked; MinimalStatsDisplay.updateDisplay(); StorageManager.saveMainSettings(); },
        handleMinimalStatsTypeDetailChange: (event) => {
            ...
            if (targetElement.type === 'checkbox') configItem.visible = targetElement.checked;
            else if (targetElement.type === 'text') configItem.label = targetElement.value.substring(0, 2);
            StorageManager.saveMinimalStatsTypeConfig(); MinimalStatsDisplay.updateDisplay();
        },
        handleMinimalStatsTypeOrderChange: (type, direction) => {
            const config = state.minimalStatsTypeDisplayConfig; const itemIndex = config.findIndex(item => item.type === type); if (itemIndex === -1) return;
            if (direction === 'up' && itemIndex > 0) [config[itemIndex], config[itemIndex - 1]] = [config[itemIndex - 1], config[itemIndex]];
            else if (direction === 'down' && itemIndex < config.length - 1) [config[itemIndex], config[itemIndex + 1]] = [config[itemIndex + 1], config[itemIndex]];
            config.forEach((item, idx) => item.order = idx); StorageManager.saveMinimalStatsTypeConfig(); MinimalStatsDisplay.updateDisplay();
            if (SettingsPanel.isInitialized() && state.settingsPanelVisible) SettingsPanel.populateMinimalStatsTypeDisplayConfigFields(); // was buildMinimalStatsTypeDisplayConfigUI
        },
        ...
        handleStatsConfigChange: (event) => {
            ...
            state.currentDepartment = CONFIG.DEPARTMENTS.CUSTOM;
            StorageManager.saveStatsConfig();
            // No need to saveDepartmentInfo here unless changing stats config *always* means custom and we want to persist that immediately
            // DepartmentManager._setDepartmentProfile(CONFIG.DEPARTMENTS.CUSTOM, false); // This already saves.
            DepartmentManager._setDepartmentProfile(CONFIG.DEPARTMENTS.CUSTOM, false); // Update department and save it
            UIManager.updateStatisticsDisplay(); MinimalStatsDisplay.updateDisplay();
            if (SettingsPanel.isInitialized() && state.settingsPanelVisible) { // check visibility
                 SettingsPanel.populateDepartmentInfo(); SettingsPanel.populateStatsConfigFields();
            }
        },
        ...
        handleStorageEvent: (event) => {
            if (event.key && event.key.startsWith(CONFIG.SCRIPT_ID_PREFIX + CONFIG.MULTI_TAB_STORAGE_PREFIX) && event.key !== (CONFIG.SCRIPT_ID_PREFIX + CONFIG.MULTI_TAB_STORAGE_PREFIX + state.currentTabId)) {
                if (CONFIG.DEBUG_MODE) Utils.debug("EventHandlers: Storage event for other tab data received:", event.key, "Value:", event.newValue);
                EventHandlers._updateFromStorageDebounced();
            }
            if (event.key === StorageManager._prefixKey(CONFIG.STORAGE_KEY_DEPARTMENT_INFO)) {
                const newDeptInfo = JSON.parse(event.newValue);
                if (newDeptInfo) {
                    let changed = false;
                    if (newDeptInfo.department && newDeptInfo.department !== state.currentDepartment) {
                        state.departmentInfo.department = newDeptInfo.department;
                        state.currentDepartment = newDeptInfo.department;
                        changed = true;
                    }
                    if (newDeptInfo.sequence && JSON.stringify(newDeptInfo.sequence) !== JSON.stringify(state.departmentInfo.sequence)) {
                         state.departmentInfo.sequence = newDeptInfo.sequence;
                         changed = true;
            ...
            if (event.key === StorageManager._prefixKey(CONFIG.STORAGE_KEY_LANGUAGE)) {
                ...
                if (newLang && newLang !== state.currentLanguage && CONFIG.AVAILABLE_LANGUAGES.find(l=>l.code === newLang)) {
                    state.currentLanguage = newLang; // Update state first
                    if (state.initialized) UIManager.refreshUIForLanguageChange(); // Then update UI
                }
            }
            if (event.key === StorageManager._prefixKey(CONFIG.STORAGE_KEY_THEME_PREFERENCE)) {
                ...
                if (newTheme && newTheme !== state.currentTheme && CONFIG.THEMES[newTheme]) {
                    state.currentTheme = newTheme; // Update state first
                    if (state.initialized) UIManager.applyThemeStyles(); // Then update UI
                }
            }
            if (event.key === StorageManager._prefixKey(CONFIG.STORAGE_KEY_STATS_CONFIG)) {
                ...
                if (newStatsConfig && JSON.stringify(newStatsConfig) !== JSON.stringify(state.statsConfig)) {
                    if (CONFIG.DEBUG_MODE) Utils.debug("EventHandlers: STORAGE_KEY_STATS_CONFIG changed by other tab. Old:", state.statsConfig, "New:", newStatsConfig);
                    state.statsConfig = newStatsConfig; // Update state first
                    state.currentDepartment = CONFIG.DEPARTMENTS.CUSTOM; // Mark as custom

                    if(state.initialized) { // Then update UI
                        UIManager.updateStatisticsDisplay();
                        MinimalStatsDisplay.updateDisplay();
                        if(SettingsPanel.isInitialized() && state.settingsPanelVisible) { // Check visibility
                            SettingsPanel.populateStatsConfigFields();
            ...
            if (event.key === StorageManager._prefixKey(CONFIG.STORAGE_KEY_MINIMAL_STATS_TYPE_CONFIG)) {
                try {
                    ...
                    if (newMinStatsTypeConfig && Array.isArray(newMinStatsTypeConfig) && JSON.stringify(newMinStatsTypeConfig) !== JSON.stringify(state.minimalStatsTypeDisplayConfig)) {
                        if (CONFIG.DEBUG_MODE) Utils.debug("EventHandlers: STORAGE_KEY_MINIMAL_STATS_TYPE_CONFIG changed. Old:", state.minimalStatsTypeDisplayConfig, "New:", newMinStatsTypeConfig);
                        const defaultConfig = MinimalStatsDisplay.getDefaultTypeConfig();
                        state.minimalStatsTypeDisplayConfig = defaultConfig.map(defaultItem => { // Update state first
                ...
                } catch (e) { Utils.error("Error processing minimalStatsTypeDisplayConfig from storage event:", e); }
            }
        }
    };
    ...
    function injectGlobalStyles() {
        ...
        themeStyleEl.textContent = `
            .ph-settings-panel::-webkit-scrollbar, .ph-right-pane::-webkit-scrollbar, .ph-stats-config-scrollable::-webkit-scrollbar, #${CONFIG.SCRIPT_ID_PREFIX}triggerDebugDisplay::-webkit-scrollbar { width: 8px; height: 8px; }
            .ph-settings-panel::-webkit-scrollbar-track, .ph-right-pane::-webkit-scrollbar-track, .ph-stats-config-scrollable::-webkit-scrollbar-track, #${CONFIG.SCRIPT_ID_PREFIX}triggerDebugDisplay::-webkit-scrollbar-track { background: ${scrollbarTrack}; border-radius:4px; }
            .ph-settings-panel::-webkit-scrollbar-thumb, .ph-right-pane::-webkit-scrollbar-thumb, .ph-stats-config-scrollable::-webkit-scrollbar-thumb, #${CONFIG.SCRIPT_ID_PREFIX}triggerDebugDisplay::-webkit-scrollbar-thumb { background: ${scrollbarThumb}; border-radius:4px;}
            .ph-settings-panel::-webkit-scrollbar-thumb:hover, .ph-right-pane::-webkit-scrollbar-thumb:hover, .ph-stats-config-scrollable::-webkit-scrollbar-thumb:hover, #${CONFIG.SCRIPT_ID_PREFIX}triggerDebugDisplay::-webkit-scrollbar-thumb:hover { background: ${scrollbarThumbHover}; }
