...
(function() {
    ...
    const CONFIG = {
        // --- General & Version ---
        SCRIPT_VERSION: '4.1',
        SCRIPT_ID_PREFIX: 'prodHelper_v4_1_',
        ...
        THEMES: {
            light: {
                ...
                MINIMAL_STATS_TEXT_COLOR: 'rgba(40, 40, 40, 0.85)',
                MINIMAL_STATS_BACKGROUND_COLOR: 'rgba(240, 240, 240, 0.0)', // Keep transparent for minimal stats
            ...
            dark: { 
                ...
                MINIMAL_STATS_TEXT_COLOR: 'rgba(210, 210, 215, 0.75)',
                MINIMAL_STATS_BACKGROUND_COLOR: 'rgba(40, 40, 50, 0.0)', // Keep transparent for minimal stats
        ...
        MINIMAL_STATS_SHOW_SECONDARY_DEFAULT: true,
        ...
        DEFAULT_LUNCH_OPTIONS: [
            { text_key: "lunch_day1", start: "11:20", end: "11:50", type: "day" }, { text_key: "lunch_day2", start: "11:50", end: "12:20", type: "day" },
            { text_key: "lunch_day3", start: "12:20", end: "12:50", type: "day" }, { text_key: "lunch_day4", start: "12:50", end: "13:20", type: "day" },
            { text_key: "lunch_night1", start: "23:20", end: "23:50", type: "night" }, { text_key: "lunch_night2", start: "23:50", end: "00:20", type: "night" },
            { text_key: "lunch_night3", start: "00:20", end: "00:50", type: "night" }, { text_key: "lunch_night4", start: "00:50", end: "01:20", type: "night" },
        ...
        STORAGE_KEY_UI_SIZE_POS: 'uiSizePos',
        STORAGE_KEY_MINIMAL_STATS_POS: 'minimalStatsPos',
        STORAGE_KEY_DEPARTMENT_INFO: 'departmentInfo',
        STORAGE_KEY_LANGUAGE: 'languagePref_global',
        STORAGE_KEY_THEME_PREFERENCE: 'themePref_global_v41',
        STORAGE_KEY_STATS_CONFIG: 'statsConfig_global_v41',
        ...
        LANG_STRINGS: { /* ... same as v4.0, with new keys added for theming and stats config ... */
            en: {
                ...
                statsConfig_tabType_CRET: "C-RET Tab", statsConfig_tabType_WHD: "WHD Tab", statsConfig_tabType_REFURB: "REFURB Tab", statsConfig_tabType_UNKNOWN: "Other Tabs",
                minimalStats_showSecondary: "Show secondary info items",
                automation_tabContributesToAggregation: "This Tab's data is available for aggregation",
                // ... other v4.0 strings copied ...
            ...
            pl: { /* ... same format, Polish translations including new keys ... */
                ...
                statsConfig_tabType_CRET: "Karta C-RET", statsConfig_tabType_WHD: "Karta WHD", statsConfig_tabType_REFURB: "Karta REFURB", statsConfig_tabType_UNKNOWN: "Inne Karty",
                minimalStats_showSecondary: "Pokaż sztuki z informacji dodatkowej",
    ...
    const state = { /* ... same as v4.0, with theme and statsConfig added ... */
        ...
        minimalStatsShowTotal: CONFIG.MINIMAL_STATS_SHOW_TOTAL_DEFAULT, minimalStatsShowPerHour: CONFIG.MINIMAL_STATS_SHOW_PER_HOUR_DEFAULT, minimalStatsShowSecondary: CONFIG.MINIMAL_STATS_SHOW_SECONDARY_DEFAULT,
    ...
    const Utils = { /* ... same as v4.0, with getThemeColor added ... */
        log: (level, ...args) => {
        ...
        createDOMElement: (tag, attributes = {}, children = []) => {
            ...
            for (const key in attributes) {
                ...
                else if (['textContent', 'innerHTML', 'value', 'checked', 'disabled', 'type', 'title', 'placeholder', 'tabIndex', 'src', 'className', 'name', 'htmlFor', 'colSpan', 'rowSpan', 'target', 'rel'].includes(key) ) element[key] = attributes[key];
    ...
    const StorageManager = { /* ... same as v4.0, ensures new keys (theme, statsConfig) are saved/loaded ... */
        ...
        loadAllState: () => {
            ...
            state.minimalStatsVisible = typeof mainSettings.minimalStatsVisible === 'boolean' ? mainSettings.minimalStatsVisible : CONFIG.MINIMAL_STATS_DEFAULT_VISIBLE; state.minimalStatsShowTotal = typeof mainSettings.minimalStatsShowTotal === 'boolean' ? mainSettings.minimalStatsShowTotal : CONFIG.MINIMAL_STATS_SHOW_TOTAL_DEFAULT; state.minimalStatsShowPerHour = typeof mainSettings.minimalStatsShowPerHour === 'boolean' ? mainSettings.minimalStatsShowPerHour : CONFIG.MINIMAL_STATS_SHOW_PER_HOUR_DEFAULT; state.minimalStatsShowSecondary = typeof mainSettings.minimalStatsShowSecondary === 'boolean' ? mainSettings.minimalStatsShowSecondary : CONFIG.MINIMAL_STATS_SHOW_SECONDARY_DEFAULT;
    ...
    const DepartmentManager = { /* ... same as v4.0, but applies default stats config ... */
        ...
        _setDepartmentProfile: (dept, applyDefaultStats = true) => {
            ...
            if (state.initialized) { UIManager.updateStatisticsDisplay(); MinimalStatsDisplay.updateDisplay(); if (state.settingsPanelVisible) { SettingsPanel.populateDepartmentInfo(); SettingsPanel.populateStatsConfigFields(); }}
        },
    ...
    const ShiftManager = { /* ... same as v4.0 ... */
    ...
    const ResizableItemsManager = { /* ... same as v4.0 ... */
    ...
    const MinimalStatsDisplay = { /* ... same as v4.0, with theme updates ... */
        isInitialized: false, dragHandle: null,
        init: () => { state.dom.minimalStatsContainer = Utils.createDOMElement('div', { id: CONFIG.MINIMAL_STATS_ID_SUFFIX, className: 'ph-minimal-stats ph-non-selectable', style: { position: 'fixed', padding: '5px 8px', borderRadius: '4px', zIndex: '2147483638', pointerEvents: 'none', userSelect: 'none', transition: 'opacity 0.3s, transform 0.3s, background-color 0.2s, color 0.2s', whiteSpace: 'nowrap', lineHeight: '1.3', textAlign: 'right', }}); state.dom.minimalStatsContent = Utils.createDOMElement('span', {textContent: 'Loading...'}); state.dom.minimalStatsContainer.appendChild(state.dom.minimalStatsContent); MinimalStatsDisplay.dragHandle = Utils.createDOMElement('div', { className: 'ph-minimal-stats-drag-handle ph-interactive', style: { position: 'absolute', top: '-5px', left: '50%', transform: 'translateX(-50%)', padding: '1px 3px', fontSize: '0.7em', borderRadius: '3px', cursor: 'move', display: 'none', pointerEvents: 'auto', userSelect: 'none', }}); document.body.appendChild(state.dom.minimalStatsContainer); MinimalStatsDisplay.setPosition(state.minimalStatsPosition); MinimalStatsDisplay.setVisibility(state.minimalStatsVisible); MinimalStatsDisplay.updateTheme(); MinimalStatsDisplay.updateDisplay(); MinimalStatsDisplay.isInitialized = true; },
        updateTheme: () => { if (!MinimalStatsDisplay.isInitialized || !state.dom.minimalStatsContainer) return; state.dom.minimalStatsContainer.style.backgroundColor = Utils.getThemeColor('MINIMAL_STATS_BACKGROUND_COLOR'); state.dom.minimalStatsContainer.style.color = Utils.getThemeColor('MINIMAL_STATS_TEXT_COLOR'); if (MinimalStatsDisplay.dragHandle) { MinimalStatsDisplay.dragHandle.style.backgroundColor = `${Utils.getThemeColor('MAIN_ACCENT_COLOR')}99`; MinimalStatsDisplay.dragHandle.style.color = Utils.getThemeColor('SETTINGS_PANEL_TEXT_COLOR'); }},
        updateDisplay: () => {
            if (!MinimalStatsDisplay.isInitialized || !state.minimalStatsVisible || !state.dom.minimalStatsContent) return; if (!state.shiftStartTime || state.currentDepartment === CONFIG.DEPARTMENTS.DETERMINING) { state.dom.minimalStatsContent.textContent = Utils.getLangString('determiningDepartment'); return; }
            const effectiveWorkMs = ShiftManager.calculateEffectiveWorkTimeMs(); const hoursWorked = effectiveWorkMs / (1000 * 60 * 60); const T = Utils.getLangString; const aggregatedCounts = StatsCalculator.getAggregatedCounts(); const primaryTotal = aggregatedCounts.primary.TOTAL; const itemsPerHour = (hoursWorked > 0.001 && primaryTotal > 0) ? (primaryTotal / hoursWorked) : 0;
            let displayTextParts = []; if (state.minimalStatsShowTotal) displayTextParts.push(T('stats_format_total_items_bare', aggregatedCounts.primary, aggregatedCounts.secondary, T)); if (state.minimalStatsShowPerHour) displayTextParts.push(`${itemsPerHour.toFixed(1)}${T('stats_perHour')}`);
            if (state.minimalStatsShowSecondary && aggregatedCounts.secondary.TOTAL > 0 && !state.minimalStatsShowTotal) { let secondaryOnlyDisplay = { CRET:0,WHD:0,REFURB:0,UNKNOWN:0,TOTAL:aggregatedCounts.secondary.TOTAL, types:aggregatedCounts.secondary.types}; if (aggregatedCounts.secondary.CRET > 0) secondaryOnlyDisplay.CRET = aggregatedCounts.secondary.CRET; if (aggregatedCounts.secondary.WHD > 0) secondaryOnlyDisplay.WHD = aggregatedCounts.secondary.WHD; if (aggregatedCounts.secondary.REFURB > 0) secondaryOnlyDisplay.REFURB = aggregatedCounts.secondary.REFURB; if (aggregatedCounts.secondary.UNKNOWN > 0) secondaryOnlyDisplay.UNKNOWN = aggregatedCounts.secondary.UNKNOWN; displayTextParts.push( T('stats_format_total_items_bare', { CRET:0,WHD:0,REFURB:0,UNKNOWN:0,TOTAL:0, types:[] }, secondaryOnlyDisplay, T).replace(/^\[0\s\w+\]/, '') );} // Show only secondary part if primary not shown.
            state.dom.minimalStatsContent.textContent = displayTextParts.join(' | ') || DepartmentManager.getDepartmentDisplayName();
        },
        setVisibility: (visible) => { state.minimalStatsVisible = visible; if (state.dom.minimalStatsContainer) { state.dom.minimalStatsContainer.style.display = visible ? 'inline-block' : 'none'; if(visible) MinimalStatsDisplay.updateDisplay(); } if(MinimalStatsDisplay.dragHandle) MinimalStatsDisplay.dragHandle.style.display = 'none'; },
        setPosition: (posObject) => { if (state.dom.minimalStatsContainer && posObject) { state.dom.minimalStatsContainer.style.right = posObject.right || CONFIG.MINIMAL_STATS_DEFAULT_RIGHT_VW + 'vw'; state.dom.minimalStatsContainer.style.bottom = posObject.bottom || CONFIG.MINIMAL_STATS_DEFAULT_BOTTOM_VH + 'vh'; state.dom.minimalStatsContainer.style.left = ''; state.dom.minimalStatsContainer.style.top = ''; }},
        showDragHandle: (show) => { if (!MinimalStatsDisplay.dragHandle || !state.dom.minimalStatsContainer) return; if (show) { if (!MinimalStatsDisplay.dragHandle.parentElement) { state.dom.minimalStatsContainer.appendChild(MinimalStatsDisplay.dragHandle); ResizableItemsManager.initMinimalStatsDragging(state.dom.minimalStatsContainer, MinimalStatsDisplay.dragHandle); } MinimalStatsDisplay.dragHandle.textContent = Utils.getLangString(CONFIG.MINIMAL_STATS_DRAG_HANDLE_TEXT_KEY); MinimalStatsDisplay.dragHandle.title = Utils.getLangString('minimalStats_editPosition'); MinimalStatsDisplay.dragHandle.style.display = 'block'; } else MinimalStatsDisplay.dragHandle.style.display = 'none'; },
        applyDraggableStyling: (isDragging) => { if (state.dom.minimalStatsContainer) { state.dom.minimalStatsContainer.style.border = isDragging ? `1px dashed ${Utils.getThemeColor('MAIN_ACCENT_COLOR')}` : 'none'; const baseBg = Utils.getThemeColor('MINIMAL_STATS_BACKGROUND_COLOR'); state.dom.minimalStatsContainer.style.backgroundColor = isDragging ? baseBg.replace(/,\s*\d(\.\d+)?\)/, ', 0.3)') : baseBg; }}
    };

    const StatsCalculator = { /* ... same as v4.0, uses state.statsConfig ... */
        getAggregatedCounts: () => {
            const primaryCounts = { CRET: 0, WHD: 0, REFURB: 0, UNKNOWN: 0, TOTAL: 0, types: [...state.statsConfig.primaryContributingTabs] }; const secondaryCounts = { CRET: 0, WHD: 0, REFURB: 0, UNKNOWN: 0, TOTAL: 0, types: [...state.statsConfig.secondaryInfoTabs] }; const allTabsRawCounts = { CRET: 0, WHD: 0, REFURB: 0, UNKNOWN: 0 };
            for (const tabId in state.otherTabsData) { const tab = state.otherTabsData[tabId]; if (!tab.contributesToAggregation && tab.tabId !== state.currentTabId) continue; const clicks = parseInt(tab.clicks, 10) || 0; const tabType = Utils.normalizeTabType(tab.type); if(allTabsRawCounts.hasOwnProperty(tabType)) allTabsRawCounts[tabType] += clicks; if (state.statsConfig.primaryContributingTabs.includes(tabType)) { if(primaryCounts.hasOwnProperty(tabType)) primaryCounts[tabType] += clicks; primaryCounts.TOTAL += clicks; } if (state.statsConfig.secondaryInfoTabs.includes(tabType)) { if(secondaryCounts.hasOwnProperty(tabType)) secondaryCounts[tabType] += clicks; secondaryCounts.TOTAL += clicks; }}
            primaryCounts.types.forEach(type => { if(!primaryCounts.hasOwnProperty(type)) primaryCounts[type] = 0; }); secondaryCounts.types.forEach(type => { if(!secondaryCounts.hasOwnProperty(type)) secondaryCounts[type] = 0; });
            return { primary: primaryCounts, secondary: secondaryCounts, allTabs: allTabsRawCounts };
        }
    };

    const AutoIncrementer = { /* ... same as v4.0 ... */
        debounceTimer: null,
    ...
    UIManager = { /* ... Rest of UIManager, fully implemented ... */
        ...
        applyThemeStyles: () => {
            ...
            // Main Panel
            state.uiContainer.style.backgroundColor = T('UI_BACKGROUND_COLOR');
            state.uiContainer.style.color = T('UI_TEXT_COLOR');
            state.uiContainer.style.borderColor = T('UI_BORDER_COLOR');
            // Top Controls
            [state.dom.toggleSettingsButton, state.dom.lockUIButton].forEach(btn => { if (btn) { btn.style.color = T('UI_TEXT_COLOR'); /* btn.style.backgroundColor = T('BUTTON_DEFAULT_BG_COLOR'); */ }});
            if(state.dom.emergencyHideButton) state.dom.emergencyHideButton.style.color = T('LAST_ACTION_TIMER_WARN_COLOR'); // Keep this distinct
            // Clicker
            ...
            // Stats Pane
            if(state.dom.rightPane) state.dom.rightPane.style.color = T('UI_TEXT_COLOR');
            if(state.dom.statsTextSummary) state.dom.statsTextSummary.style.borderTopColor = T('STATS_TEXT_SUMMARY_BORDER_TOP');
            if(state.dom.triggerDebugDisplay) state.dom.triggerDebugDisplay.style.borderTopColor = T('UI_TEXT_COLOR'); // Simple contrast
            // Bottom Bar
            const bottomBar = state.uiContainer.querySelector('.ph-bottom-bar');
            if(bottomBar) bottomBar.style.borderTopColor = T('BOTTOM_BAR_BORDER_TOP');
            if(state.dom.uiTabIndicatorText) state.dom.uiTabIndicatorText.style.color = state.currentTabModeDetails.isCustom ? state.currentTabModeDetails.textColor : ( T('UI_TEXT_COLOR').replace(/,\s*\d(\.\d+)?\)/, ', 0.6)') ); // Use custom or themed semi-transparent
            if(state.dom.lastActionTimerDisplay) { state.dom.lastActionTimerDisplay.style.color = Date.now() - state.lastActionTimestampForThisTab > CONFIG.LAST_ACTION_TIMER_WARN_SECONDS * 1000 ? T('LAST_ACTION_TIMER_WARN_COLOR') : T('UI_TEXT_COLOR'); }
            if(state.dom.realTimeClock) state.dom.realTimeClock.style.color = T('UI_TEXT_COLOR');
            ...
            // Settings panel and minimal stats have their own theme update methods
            if (SettingsPanel.isInitialized()) SettingsPanel.updateTheme();
            if (MinimalStatsDisplay.isInitialized) MinimalStatsDisplay.updateTheme();
            UIManager.applyDebugPointerEventBorders(); // Re-apply with potentially new debug colors
        },
        ...
        updateUITabIndicatorText: () => { if(state.dom.uiTabIndicatorText) { const mode = state.customTabThemes[state.currentTabFullUrl] || state.currentTabModeDetails; state.dom.uiTabIndicatorText.textContent = mode.name; state.dom.uiTabIndicatorText.style.color = mode.isCustom ? mode.textColor : Utils.getThemeColor('UI_TEXT_COLOR').replace(/,\s*\d(\.\d+)?\)/, ', 0.6)'); }},
        ...
        updateStatisticsDisplay: () => {
            ...
            if (state.currentDepartment === CONFIG.DEPARTMENTS.DETERMINING && !StorageManager.load(CONFIG.STORAGE_KEY_STATS_CONFIG, null, false) /*Only show determining if no custom config exists*/ ) { state.dom.statsTextSummary.innerHTML = `<p>${Utils.getLangString('determiningDepartment')}</p>`; return; }
    ...
    SettingsPanel = { /* ... Rest of SettingsPanel, fully implemented with theme and stats config ... */
        _isInitialized: false,
        ...
        updateTheme: () => { if (!SettingsPanel.isInitialized() || !state.dom.settingsPanel) return; const T = Utils.getThemeColor; state.dom.settingsPanel.style.backgroundColor = T('SETTINGS_PANEL_BACKGROUND'); state.dom.settingsPanel.style.borderLeft = `2px solid ${T('SETTINGS_PANEL_BORDER_COLOR')}`; SettingsPanel.rebuildOrRelocalize(); /* Rebuild to apply text/input colors */ },
        rebuildOrRelocalize: () => {
            ...
            const commonLabelStyle = { display: 'block', marginBottom: '2px', fontSize: '0.8em', color: TC('SETTINGS_PANEL_TEXT_COLOR') }; // Text color from theme
            ...
            // Theme Selector
            state.dom.settingsThemeSelect = Utils.createDOMElement('select', {id: 'themeSelect', style: commonSelectStyle, className:'ph-interactive', onChange: EventHandlers.handleThemeChange });
            Object.keys(CONFIG.THEMES).forEach(themeKey => state.dom.settingsThemeSelect.add(new Option(T(`theme_${themeKey}`), themeKey)));
            state.dom.settingsPanel.appendChild(createSection(null, [Utils.createDOMElement('label', {htmlFor: state.dom.settingsThemeSelect.id, textContent: T('theme'), style: commonLabelStyle}), state.dom.settingsThemeSelect ]));
            ...
            const statsConfigElements = [];
            const buildStatsCheckbox = (typeKey, listKey) => {
                const cbId = `statsCfg_${listKey}_${typeKey}`;
                state.dom[cbId] = Utils.createDOMElement('input', {type:'checkbox', id: cbId, style: checkboxStyle, className:'ph-interactive ph-stats-config-cb', dataset: { tabtype: typeKey, listkey: listKey }, onChange: EventHandlers.handleStatsConfigChange });
                return Utils.createDOMElement('label', {htmlFor: cbId, textContent: T(`statsConfig_tabType_${typeKey}`), style:checkboxRowStyle, className:'ph-interactive'}, [state.dom[cbId]]);
            };
            statsConfigElements.push(Utils.createDOMElement('p', {textContent: T('statsConfig_primaryLabel'), style: commonLabelStyle}));
            [CONFIG.TAB_TYPES.CRET, CONFIG.TAB_TYPES.WHD, CONFIG.TAB_TYPES.REFURB, CONFIG.TAB_TYPES.UNKNOWN].forEach(type => statsConfigElements.push(buildStatsCheckbox(type, 'primaryContributingTabs')));
            statsConfigElements.push(Utils.createDOMElement('p', {textContent: T('statsConfig_secondaryLabel'), style: {...commonLabelStyle, marginTop:'8px'}}));
            [CONFIG.TAB_TYPES.CRET, CONFIG.TAB_TYPES.WHD, CONFIG.TAB_TYPES.REFURB, CONFIG.TAB_TYPES.UNKNOWN].forEach(type => statsConfigElements.push(buildStatsCheckbox(type, 'secondaryInfoTabs')));
            state.dom.settingsPanel.appendChild(createSection('statsConfig_title', statsConfigElements));
            ...
            state.dom.minimalStatsShowSecondaryCheckbox = Utils.createDOMElement('input', {type:'checkbox', id:'minimalStatsShowSecondaryCb', style: checkboxStyle, className:'ph-interactive', onChange: EventHandlers.handleMinimalStatsDetailChange}); minimalStatsElements.push(Utils.createDOMElement('label', {htmlFor: state.dom.minimalStatsShowSecondaryCheckbox.id, textContent: T('minimalStats_showSecondary'), style:checkboxRowStyle, className:'ph-interactive'}, [state.dom.minimalStatsShowSecondaryCheckbox]));
        ...
        populateAllFields: () => {
            ...
            if(state.dom.minimalStatsEnabledCheckbox) state.dom.minimalStatsEnabledCheckbox.checked = state.minimalStatsVisible; if(state.dom.minimalStatsShowTotalCheckbox) state.dom.minimalStatsShowTotalCheckbox.checked = state.minimalStatsShowTotal; if(state.dom.minimalStatsShowPerHourCheckbox) state.dom.minimalStatsShowPerHourCheckbox.checked = state.minimalStatsShowPerHour; if(state.dom.minimalStatsShowSecondaryCheckbox) state.dom.minimalStatsShowSecondaryCheckbox.checked = state.minimalStatsShowSecondary;
        ...
        populateStatsConfigFields: () => {
            ...
            [CONFIG.TAB_TYPES.CRET, CONFIG.TAB_TYPES.WHD, CONFIG.TAB_TYPES.REFURB, CONFIG.TAB_TYPES.UNKNOWN].forEach(type => {
                const primaryCb = state.dom[`statsCfg_primaryContributingTabs_${type}`]; if (primaryCb) primaryCb.checked = state.statsConfig.primaryContributingTabs.includes(type);
                const secondaryCb = state.dom[`statsCfg_secondaryInfoTabs_${type}`]; if (secondaryCb) secondaryCb.checked = state.statsConfig.secondaryInfoTabs.includes(type);
            });
        },
    ...
    EventHandlers = { /* ... Rest of EventHandlers, with new theme and stats config handlers ... */
        ...
        handleMinimalStatsDetailChange: (event) => { const checkbox = event.target; if (checkbox === state.dom.minimalStatsShowTotalCheckbox) state.minimalStatsShowTotal = checkbox.checked; else if (checkbox === state.dom.minimalStatsShowPerHourCheckbox) state.minimalStatsShowPerHour = checkbox.checked; else if (checkbox === state.dom.minimalStatsShowSecondaryCheckbox) state.minimalStatsShowSecondary = checkbox.checked; MinimalStatsDisplay.updateDisplay(); StorageManager.saveMainSettings(); },
        ...
        handleThemeChange: (event) => { state.currentTheme = event.target.value; StorageManager.saveThemePreference(); UIManager.applyThemeStyles(); /* SettingsPanel.rebuildOrRelocalize() called by applyThemeStyles indirectly if structure depends on it or if an explicit call in applyThemeStyles for SettingsPanel.updateTheme */ if (SettingsPanel.isInitialized()) SettingsPanel.updateTheme(); /* ensure settings panel specific theme items are updated */ },
        handleStatsConfigChange: (event) => {
            ...
            state.currentDepartment = CONFIG.DEPARTMENTS.CUSTOM; // User has customized, mark as custom
            DepartmentManager.getDepartmentDisplayName(); // Update display if settings panel is open
            StorageManager.saveStatsConfig(); UIManager.updateStatisticsDisplay(); MinimalStatsDisplay.updateDisplay();
            if (SettingsPanel.isInitialized()) SettingsPanel.populateDepartmentInfo(); // Refresh dept display to show "CUSTOM"
        },
        handleWindowBeforeUnload: () => { Utils.info("Window beforeunload: Saving state..."); StorageManager.writeCurrentTabDataToLocalStorage(); StorageManager.saveMainSettings(); StorageManager.saveUIPanelSizePos(); StorageManager.saveMinimalStatsPos(); StorageManager.saveCustomThemes(); StorageManager.saveDepartmentInfo(); StorageManager.saveLanguagePreference(); StorageManager.saveThemePreference(); StorageManager.saveStatsConfig(); },
        handleStorageEvent: (event) => { if (event.key && event.key.startsWith(CONFIG.SCRIPT_ID_PREFIX + CONFIG.MULTI_TAB_STORAGE_PREFIX) && event.key !== (CONFIG.SCRIPT_ID_PREFIX + CONFIG.MULTI_TAB_STORAGE_PREFIX + state.currentTabId)) StorageManager.readAllTabsDataFromLocalStorage(false); if (event.key === StorageManager._prefixKey(CONFIG.STORAGE_KEY_DEPARTMENT_INFO)) { const newDeptInfo = JSON.parse(event.newValue); if (newDeptInfo && newDeptInfo.department && newDeptInfo.department !== state.currentDepartment) { state.departmentInfo = newDeptInfo; state.currentDepartment = newDeptInfo.department; if(state.initialized) { UIManager.updateStatisticsDisplay(); MinimalStatsDisplay.updateDisplay(); if (SettingsPanel.isInitialized() && state.settingsPanelVisible) SettingsPanel.populateDepartmentInfo(); }} else if (newDeptInfo && newDeptInfo.sequence.length !== state.departmentInfo.sequence.length) { state.departmentInfo = newDeptInfo; if (state.currentDepartment === CONFIG.DEPARTMENTS.DETERMINING) DepartmentManager.updateSequenceAndTryDetermine(); }} if (event.key === StorageManager._prefixKey(CONFIG.STORAGE_KEY_LANGUAGE)) { const newLang = JSON.parse(event.newValue); if (newLang && newLang !== state.currentLanguage && CONFIG.AVAILABLE_LANGUAGES.find(l=>l.code === newLang)) { state.currentLanguage = newLang; if (state.initialized) UIManager.refreshUIForLanguageChange(); }} if (event.key === StorageManager._prefixKey(CONFIG.STORAGE_KEY_THEME_PREFERENCE)) { const newTheme = JSON.parse(event.newValue); if (newTheme && newTheme !== state.currentTheme && CONFIG.THEMES[newTheme]) { state.currentTheme = newTheme; if (state.initialized) UIManager.applyThemeStyles(); }} if (event.key === StorageManager._prefixKey(CONFIG.STORAGE_KEY_STATS_CONFIG)) { const newStatsConfig = JSON.parse(event.newValue); if (newStatsConfig && JSON.stringify(newStatsConfig) !== JSON.stringify(state.statsConfig)) { state.statsConfig = newStatsConfig; state.currentDepartment = CONFIG.DEPARTMENTS.CUSTOM; if(state.initialized) {UIManager.updateStatisticsDisplay(); MinimalStatsDisplay.updateDisplay(); if(SettingsPanel.isInitialized()) {SettingsPanel.populateStatsConfigFields(); SettingsPanel.populateDepartmentInfo(); }}}}}
    };
    ...
    function injectGlobalStyles() {
        ...
        // Theme-dependent scrollbar styles (optional but good for consistency)
        ...
        themeStyleEl.textContent = `
            .ph-settings-panel::-webkit-scrollbar, .ph-right-pane::-webkit-scrollbar { width: 8px; }
            .ph-settings-panel::-webkit-scrollbar-track, .ph-right-pane::-webkit-scrollbar-track { background: ${scrollbarTrack}; border-radius:4px; }
            .ph-settings-panel::-webkit-scrollbar-thumb, .ph-right-pane::-webkit-scrollbar-thumb { background: ${scrollbarThumb}; border-radius:4px;}
            .ph-settings-panel::-webkit-scrollbar-thumb:hover, .ph-right-pane::-webkit-scrollbar-thumb:hover { background: ${scrollbarThumbHover}; }
        `;
    }
