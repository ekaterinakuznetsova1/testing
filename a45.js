...
(function() {
    ...
    const CONFIG = {
        // --- General & Version ---
        SCRIPT_VERSION: '4.5', // Updated for v4.5
        SCRIPT_ID_PREFIX: 'prodHelper_v4_5_', // Updated for v4.5
        ...
        STORAGE_KEY_THEME_PREFERENCE: 'themePref_global_v45',       // Versioned for v4.5
        STORAGE_KEY_STATS_CONFIG: 'statsConfig_global_v45',         // Versioned for v4.5
        STORAGE_KEY_MINIMAL_STATS_TYPE_CONFIG: 'minimalStatsTypeConfig_global_v45', // Versioned for v4.5
        ...
        LANG_STRINGS: {
            en: {
                ...
                statsConfig_tabType_General: "Other Tabs",
            ...
            pl: {
                ...
                statsConfig_tabType_General: "Inne Karty",
    ...
    const Utils = {
        ...
        getLangString: (key, ...args) => {
            ...
            if (typeof str === 'undefined') {
                ...
                if (typeof str === 'undefined') {
                    // Return the key itself or a more specific placeholder for "soft" missing strings (like summaries)
                    // This check helps distinguish a truly missing critical string from an optional one.
                    if (key && (key.includes("depSummary_") && key.endsWith("_logic")) ) return ""; // Return empty for optional summaries
            ...
            if (args.length > 0 && typeof str === 'string' && args[0] !== null) { // Check args[0] for null to avoid issues with (key, null) calls
    ...
    const StorageManager = {
        ...
        writeCurrentTabDataToLocalStorage: () => { if (!state.currentTabId) { Utils.error("Cannot write tab data: currentTabId is not set."); return; } try { const tabData = { tabId: state.currentTabId, type: state.currentTabType, modeName: state.currentTabModeDetails.name, clicks: state.clicksForThisTab, lastActionTimestamp: state.lastActionTimestampForThisTab, contributesToAggregation: state.currentTabContributesToAggregation, department: state.currentDepartment, timestamp: Date.now() }; localStorage.setItem(CONFIG.SCRIPT_ID_PREFIX + CONFIG.MULTI_TAB_STORAGE_PREFIX + state.currentTabId, JSON.stringify(tabData)); Utils.debug(`Wrote tab data for ${state.currentTabId}: Clicks=${tabData.clicks}, Type=${tabData.type}, Dept=${tabData.department}`); } catch (e) { Utils.error('Error writing tab data to localStorage:', e); }},
        readAllTabsDataFromLocalStorage: (isInitialLoad = false) => {
            let newOtherTabsData = {}; const now = Date.now();
            Utils.debug(`Reading all tabs data. Current tab ID: ${state.currentTabId}. Initial load: ${isInitialLoad}`);
            try { for (let i = 0; i < localStorage.length; i++) { const key = localStorage.key(i); if (key && key.startsWith(CONFIG.SCRIPT_ID_PREFIX + CONFIG.MULTI_TAB_STORAGE_PREFIX)) { const itemJson = localStorage.getItem(key); if (itemJson) { try { const itemData = JSON.parse(itemJson); if (now - (itemData.timestamp || 0) > CONFIG.MULTI_TAB_DATA_TTL_MS) { Utils.debug(`Tab data for ${itemData.tabId} expired. Removing.`); localStorage.removeItem(key); continue; } if (itemData.tabId === state.currentTabId) { if (isInitialLoad) { state.clicksForThisTab = parseInt(itemData.clicks, 10) || 0; state.lastActionTimestampForThisTab = parseInt(itemData.lastActionTimestamp, 10) || Date.now(); Utils.debug(`Initial load for current tab ${state.currentTabId}: Clicks=${state.clicksForThisTab}`); } newOtherTabsData[itemData.tabId] = { ...itemData, clicks: state.clicksForThisTab, lastActionTimestamp: state.lastActionTimestampForThisTab, contributesToAggregation: state.currentTabContributesToAggregation, department: state.currentDepartment, type: state.currentTabType }; } else { newOtherTabsData[itemData.tabId] = itemData; Utils.debug(`Read data for other tab ${itemData.tabId}: Clicks=${itemData.clicks}, Type=${itemData.type}, Dept=${itemData.department}`); } } catch (parseError) { Utils.error(`Error parsing localStorage key ${key}:`, parseError); localStorage.removeItem(key); }}}}} catch (e) { Utils.error('Error reading from localStorage multi-tab sync:', e); }
            state.otherTabsData = newOtherTabsData;
            Utils.debug("Updated state.otherTabsData:", JSON.parse(JSON.stringify(state.otherTabsData))); // Deep copy for logging
    ...
    const DepartmentManager = {
        ...
        getDepartmentDisplayName: (deptCode) => {
            ...
            // 'code' here is expected to be a string like 'UG', 'BK', etc.
    ...
    MinimalStatsDisplay = {
        ...
        getDefaultTypeConfig: () => {
            return [
                ...
                { type: CONFIG.TAB_TYPES.UNKNOWN,visible: false, order: 3, label: 'U' }
            ];
        },
        updateTheme: () => {
        ...
        updateDisplay: () => {
            ...
            if (state.currentDepartment &&
                ...
                const deptLogicSummary = T(deptLogicKey, null); // Pass null to avoid error if not a function
            ...
            // Fallback display if all lines are empty
            ...
            state.dom.minimalStatsContent.innerHTML = finalDisplayParts.join('<br>');
            Utils.debug("MinimalStatsDisplay updated. Content:", state.dom.minimalStatsContent.innerHTML.replace(/<br>/g, ' | '));
        },
    ...
    const StatsCalculator = {
        getAggregatedCounts: () => {
            ...
            Utils.debug("StatsCalculator: Calculating aggregated counts. state.otherTabsData:", JSON.parse(JSON.stringify(state.otherTabsData)));
            ...
            for (const tabId in state.otherTabsData) {
                ...
                if (!tab.contributesToAggregation && tab.tabId !== state.currentTabId) {
                    Utils.debug(`StatsCalculator: Tab ${tabId} does not contribute to aggregation and is not current tab. Skipping.`);
                    continue;
                }
                ...
                Utils.debug(`StatsCalculator: Processing tab ${tabId} - Type: ${tab.type} (Normalized: ${tabType}), Clicks: ${clicks}, Contributes: ${tab.contributesToAggregation}`);
            ...
            Utils.debug("StatsCalculator: Aggregated Results - Primary:", primaryCounts, "Secondary:", secondaryCounts, "AllTabsRaw:", allTabsRawCounts);
    ...
    EventHandlers = {
        ...
        handleMinimalStatsTypeOrderChange: (type, direction) => {
            ...
            if (SettingsPanel.isInitialized() && state.settingsPanelVisible) SettingsPanel.buildMinimalStatsTypeDisplayConfigUI();
        },
        ...
        handleStatsConfigChange: (event) => {
            ...
            if (SettingsPanel.isInitialized()) { SettingsPanel.populateDepartmentInfo(); SettingsPanel.populateStatsConfigFields(); }
        },
        ...
        handleStorageEvent: (event) => {
            if (event.key && event.key.startsWith(CONFIG.SCRIPT_ID_PREFIX + CONFIG.MULTI_TAB_STORAGE_PREFIX) && event.key !== (CONFIG.SCRIPT_ID_PREFIX + CONFIG.MULTI_TAB_STORAGE_PREFIX + state.currentTabId)) {
                Utils.debug("Storage event for other tab data:", event.key);
                ...
                try {
                    ...
                    if (newMinStatsTypeConfig && Array.isArray(newMinStatsTypeConfig) && JSON.stringify(newMinStatsTypeConfig) !== JSON.stringify(state.minimalStatsTypeDisplayConfig)) {
                        ...
                        if (state.initialized) {
                            MinimalStatsDisplay.updateDisplay();
                            if (SettingsPanel.isInitialized() && state.settingsPanelVisible) SettingsPanel.populateMinimalStatsTypeDisplayConfigFields();
                        }
                    }
