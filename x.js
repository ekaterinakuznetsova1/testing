...
*/
    ...
    function handleCounterInputDynamic(event) { const INPUT_PADDING_GUESS = 20; const input = event.target; const valueLength = input.value.length; let newFontSize = CONFIG.MAIN_COUNTER_FONT_SIZE_INITIAL_EM; if (valueLength > CONFIG.MAIN_COUNTER_MAX_CHARS_BEFORE_RESIZE) { const overflowChars = valueLength - CONFIG.MAIN_COUNTER_MAX_CHARS_BEFORE_RESIZE; newFontSize = Math.max(CONFIG.MAIN_COUNTER_FONT_SIZE_MIN_EM, CONFIG.MAIN_COUNTER_FONT_SIZE_INITIAL_EM - overflowChars * 0.55); } input.style.fontSize = `${newFontSize}em`; input.style.width = `${Math.max(70, valueLength * (newFontSize * 0.65) + INPUT_PADDING_GUESS)}px`;  } // Added guess for padding
